...
import { ChevronLeft, ChevronRight, Plus, Clock, MapPin, Bell, Check } from "lucide-react";
...
export function Agenda() {
  ...
  const { upcoming, toggleDone } = useReminders();

  const appointments = [
    { id: 1, date: "24 Apr 2026", time: "15:00", title: "Controllo veterinario", location: "Clinica San Francesco", color: "bg-[var(--pastel-orange)]" },
    { id: 2, date: "26 Apr 2026", time: "10:30", title: "Toilettatura", location: "Pet Grooming Studio", color: "bg-[var(--pastel-coral)]" },
    { id: 3, date: "28 Apr 2026", time: "16:00", title: "Lezione addestramento", location: "Centro Cinofilo", color: "bg-[var(--pastel-mint)]" },
  ];
  ...
  return (
    <div className="min-h-screen bg-background pb-24">
      ...
      <div className="px-6 mt-6 space-y-6">
        ...
        {/* Reminder section — always on top */}
        {upcoming.length > 0 && (
          <div>
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                <Bell size={16} className="text-[var(--pastel-orange)]" />
                <h2>Reminder in scadenza</h2>
              </div>
              <button
            ...
            <div className="space-y-2">
              {upcoming.slice(0, 3).map((r) => {
                ...
                const daysLabel = days <= 0
                  ? days === 0 ? "Oggi" : `Scaduto da ${Math.abs(days)}gg`
                  : days === 1 ? "Domani"
                  : `Tra ${days} giorni`;
                return (
                  <div key={r.id} className={`flex items-center gap-3 p-3.5 rounded-2xl border ${urgencyBg(days)}`}>
                    <Bell size={16} className="text-muted-foreground flex-shrink-0" />
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium truncate">{r.title}</p>
                      <p className="text-xs text-muted-foreground">{r.petName} · {daysLabel}</p>
                    </div>
                    <button
                      onClick={() => toggleDone(r.id)}
                      className="w-8 h-8 rounded-xl bg-white/70 border border-border/50 flex items-center justify-center flex-shrink-0"
                    >
                      <Check size={14} />
                    </button>
                  </div>
              ...
              {upcoming.length > 3 && (
                ...
                >
                  +{upcoming.length - 3} altri reminder
                </button>
              )}
            </div>
          </div>
        ...
        <div>
          ...
          <div className="space-y-4">
            {appointments.map((appointment) => (
              <div key={appointment.id} className="p-5 rounded-3xl bg-card shadow-sm border border-border">
            ...
            <div className="space-y-5">
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <button onClick={() => setEventType("appointment")} className={`p-4 rounded-xl transition-all ${eventType === "appointment" ? "bg-[var(--pastel-orange)] text-white" : "bg-muted"}`}>
                    Appuntamento
                  </button>
                  <button onClick={() => setEventType("reminder")} className={`p-4 rounded-xl transition-all ${eventType === "reminder" ? "bg-[var(--pastel-orange)] text-white" : "bg-muted"}`}>
                    Reminder
                  </button>
                </div>
              </div>
              ...
              <div>
                ...
                <input id="event-title" type="text" placeholder={eventType === "appointment" ? "es. Visita veterinaria" : "es. Vaccino antirabbia"} className="w-full px-4 py-3 rounded-xl bg-muted border-0" />
              </div>

              <div>
                <label className="text-sm text-muted-foreground mb-3 block">{eventType === "appointment" ? "Data e ora" : "Data"}</label>
                <input id="event-datetime" type={eventType === "appointment" ? "datetime-local" : "date"} className="w-full px-4 py-3 rounded-xl bg-muted border-0" />
              </div>

              {eventType === "appointment" && (
                <div>
                  <label className="text-sm text-muted-foreground mb-3 block">Luogo</label>
                  <input id="event-location" type="text" placeholder="es. Clinica Veterinaria" className="w-full px-4 py-3 rounded-xl bg-muted border-0" />
                </div>
              )}
              ...
              {eventType === "reminder" && (
                <div>
                  ...
                  <textarea id="event-notes" placeholder="es. Portare libretto sanitario" className="w-full px-4 py-3 rounded-xl bg-muted border-0 min-h-[80px]" />
                </div>
              )}
              ...
              <div className="pt-2">
                ...
                <button onClick={() => setShowAddModal(false)} className="w-full py-4 rounded-2xl bg-gradient-to-r from-[var(--pastel-orange)] to-[var(--pastel-coral)] text-white shadow-lg">
                  Salva nell'app
                </button>
              </div>
            </div>
